...
Backfill.prototype.expand = function (doc, expand) {
  ...
  // TODO: proposed enhancement, expand can be store in cache
  // where the value is the the calculated 'emap'. with this
  // we can skip the complex computation if it is the same expand param
  ...
  expands.sort().reverse().forEach(subj => {
    let match = false
    let xSubj = subj.replace(/\*/g, '_')
    let re = new RegExp(`^${xSubj}\\.|^${xSubj}$`)

    for (let i = 0; i < neutralized.length; i++) {
      if (re.test(neutralized[i].replace(/\*/g, '_'))) {
        match = true; break
      }
    }
