...
Backfill.prototype.expand = function (doc, expand) {
  ...
  expands.sort().reverse().forEach(subj => {
    ...
    for (let i = 0; i < neutralized.length; i++) {
      ...
      console.log('---y', xSubj, neutralized[i].replace(/\*/g, '_'))
      if (re.test(neutralized[i].replace(/\*/g, '_'))) {
        console.log('---z', xSubj, neutralized[i].replace(/\*/g, '_'))
        match = true; break
      }
    }
    ...
    if (!match) {
      console.log('---u', subj)
      neutralized.push(subj)
    }
  })

  console.log('--b', neutralized)
